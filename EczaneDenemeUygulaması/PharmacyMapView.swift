...
struct PharmacyMapView: View {
    ...
    var body: some View {
        Map(coordinateRegion: $region, annotationItems: [pharmacy]) { pharmacy in
            // Marker için doğru kullanım
            MapMarker(coordinate: Self.getCoordinate(from: pharmacy.loc), tint: .blue)
        }
        .navigationTitle(pharmacy.name ?? "Eczane")
        .navigationBarTitleDisplayMode(.inline)
    }

    private static func getCoordinate(from loc: String?) -> CLLocationCoordinate2D {
